<x-app-layout>
  ...
  <select  name="user_id" class="form-select" id="userSelect" aria-label="Floating label select example" disabled>
    <option selected>Select The User</option>
    @if ($users->count() > 0)
      @foreach ($users as $user)
        <option @if ($user->id == $selecteduser->id)
            selected
        @endif 
...
<br>
  <select  name="role_id[]"   class="form-select" id="roleSelect" aria-label="Floating label select example">
    ...
    @endif
                        ...
                        <button type="submit" class="btn btn-primary mt-3">Update Assign Role To User</button>
                    </form>
                </div>
            </div>
        </div>
